...
| 2026-09-17 | Generic action functions return the resource the action returns, not the owner; breaking, 0.2.0 (#87) |
...
| Issue | What | Why now |
| ----- | ---- | ------- |
| #88 | `Rpc.Runner` answers a generic action with the owner's fields when `fields` is absent | Since #87 that response decodes to nulls without an error; due before 0.2.0 |
